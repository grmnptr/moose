...
// MOOSE includes
#include "LibtorchNeuralNetTransfer.h"
#include "LibtorchNeuralNetControl.h"

registerMooseObject("StochasticToolsApp", LibtorchNeuralNetTransfer);

InputParameters
LibtorchNeuralNetTransfer::validParams()
{
...
LibtorchNeuralNetTransfer::LibtorchNeuralNetTransfer(const InputParameters & parameters)
  : MultiAppTransfer(parameters),
    SurrogateModelInterface(this),
    _control_name(getParam<std::string>("control_name")),
    _trainer(getSurrogateTrainerByName<LibtorchSimpleNNControlTrainer>(
...
LibtorchNeuralNetTransfer::execute()
{
  ...
  switch (_direction)
  {
    case FROM_MULTIAPP:
      ...
      mooseError("This tranfer doesn ot support transfer from the multiapps.");
      break;
